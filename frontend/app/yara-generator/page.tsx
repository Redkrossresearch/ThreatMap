...
export default function YaraRuleGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    setError("");
    try {
      const response = await fetch(process.env.NEXT_PUBLIC_API_URL + "/api/v1/generators/yara", {
        method: "POST",
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `${ruleData.malware_name || "rule"}.yar`;
    a.click();
  ...
  return (
    <div className="relative min-h-full flex flex-col gap-6">
      <div className="flex items-center justify-between z-10">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white mb-2">YARA Rule Generator</h1>
          <p className="text-on-surface-variant max-w-2xl text-sm">
            Generate AI-powered YARA rules from malware intelligence and IOCs.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 z-10">
        ...
        >
          <h2 className="text-xl font-semibold text-white mb-6 border-b border-white/5 pb-2">Malware Information</h2>
          
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-semibold text-on-surface-variant mb-1 uppercase tracking-wider">File Hash (MD5, SHA1, SHA256)</label>
              <input 
                type="text" name="file_hash" value={ruleData.file_hash} onChange={handleInputChange}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-semibold text-on-surface-variant mb-1 uppercase tracking-wider">Malware Name</label>
                <input 
                  type="text" name="malware_name" value={ruleData.malware_name} onChange={handleInputChange}
                  className="w-full bg-surface border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all"
                  placeholder="e.g., WannaCry"
                />
              </div>
              <div>
                <label className="block text-xs font-semibold text-on-surface-variant mb-1 uppercase tracking-wider">Malware Family</label>
                <input 
                  type="text" name="malware_family" value={ruleData.malware_family} onChange={handleInputChange}
                  className="w-full bg-surface border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all"
            ...
            <div>
              <label className="block text-xs font-semibold text-on-surface-variant mb-1 uppercase tracking-wider">IOC Information</label>
              <textarea 
                name="ioc_information" value={ruleData.ioc_information} onChange={handleInputChange} rows={3}
                ...
                placeholder="Domains, IPs, Mutexes, Registry Keys..."
              />
            </div>

            <div>
              <label className="block text-xs font-semibold text-on-surface-variant mb-1 uppercase tracking-wider">Behavior Description</label>
              <textarea 
                name="behavior_description" value={ruleData.behavior_description} onChange={handleInputChange} rows={3}
                className="w-full bg-surface border border-white/10 rounded-lg px-3 py-2 text-sm text-white focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all resize-none"
                placeholder="What does the malware do?"
              />
            </div>
          ...
          <div className="flex-1 bg-transparent overflow-y-auto">
            {error ? (
              ...
              <div className="h-full flex items-center justify-center text-on-surface-variant text-sm flex-col gap-3 opacity-50">
                ...
                <p>Fill out the form and click Generate to see the YARA rule here.</p>
              </div>
            )}
          </div>
        </motion.div>
